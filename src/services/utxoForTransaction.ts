...
type UtxoForTransactionRequestType = {
...
export const utxoForTransaction =
  (pool: Pool) => async (req: Request, res: Response<Array<UtxoInfo>>) => {
    ...
    const whereConditions = req.body.transactions
      .map(
        (transaction: TransactionIdType): string =>
          `(index = ($${transaction.index})::smallint and hash = ($${transaction.txHash})::varchar )`
      )
      .join(" or ");
    ...
    const result = await pool.query(utxoForTransactionQuery);
